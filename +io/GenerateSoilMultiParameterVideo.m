function GenerateSoilMultiParameterVideo(outputPath, moistureData, temperatureData, pressureData, rainfallData, current_date, SUMDELTZ)
    % 土壤多参数变化视频生成函数
    % 输入参数：
    %   outputPath - 输出视频文件路径
    %   moistureData - 土壤水分数据矩阵（128x461）
    %   temperatureData - 土壤温度数据矩阵（128x461）
    %   pressureData - 土壤气压数据矩阵（128x461）
    ...
    temperature_max = max(temperatureData(:)); % 土壤温度最大值
    pressure_max = max(pressureData(:));     % 土壤气压最大值
    ...
    try
        ...
        for dayIdx = 1:size(moistureData, 2)
            ...
            currentTemperatureData = temperatureData(:, dayIdx);
            currentPressureData = pressureData(:, dayIdx);
            ...
            xlabel(ax2, '土壤温度 (°C)', 'FontSize', 10, 'FontWeight', 'bold');
            ylabel(ax2, '土壤深度 (cm)', 'FontSize', 10, 'FontWeight', 'bold');
            title(ax2, '土壤温度剖面', 'FontSize', 12, 'FontWeight', 'bold');
            xlim(ax2, [0, temperature_max]);
            ylim(ax2, [0, depth_max]);
            grid(ax2, 'on');
            set(ax2, 'GridAlpha', 0.3);
            set(ax2, 'XTick', linspace(0, temperature_max, 5));
            ...
            xlabel(ax3, '土壤气压 (kPa)', 'FontSize', 10, 'FontWeight', 'bold');
            ylabel(ax3, '土壤深度 (cm)', 'FontSize', 10, 'FontWeight', 'bold');
            title(ax3, '土壤气压剖面', 'FontSize', 12, 'FontWeight', 'bold');
            xlim(ax3, [0, pressure_max*1.1]);
            ylim(ax3, [0, depth_max]);
            grid(ax3, 'on');
            set(ax3, 'GridAlpha', 0.3);
            set(ax3, 'XTick', 0:10:pressure_max*1.1);
