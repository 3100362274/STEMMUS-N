...
global EX ML NL ND Theta_L XOLD J Theta_LL XWRE IH Theta_s XK
...
% keep running like that. Otherwise, change trend from wetting to drying.    
    if IH(ML)==1 && XOLD(ML)<=(EX+Criteria_zero)  
        XOLD(ML)=EX;
        return
    elseif IH(ML)==2                     
        if XOLD(ML)>=(EX-Criteria_zero)
            XOLD(ML)=EX;
            return
        else
            IH(ML)=1;
            for ND=1:2
                if (Theta_s(J)-Theta_LL(ML,ND))<Criteria_zero
        ...
        IH(ML)=2;
        for ND=1:2
            if (Theta_LL(J)-XK(J))<Criteria_zero
                XWRE(ML,ND)=XK(J);
            else
