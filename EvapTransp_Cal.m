...
global PT_PM_0 PT_PM_VEG  PME P Ts_msr 
% Set constants
...
Z=1051;             % altitute of the location(m)
...
Mdata=xlsread('E:\朱\Modle Information\STEMMUS\STEMMUS_wanhua\Meterology data','Sheet1','b4:R366');
Tmax=Mdata(:,1);
Tmin=Mdata(:,2);
Tm=Mdata(:,3);
RHa=Mdata(:,4);
...
%P=Mdata(:,16);
Ts_msr=Mdata(:,17);
% h_SUR_msr=Mdata(:,18);  %地表随时间变化水头值

% Calculation procedure  以日变化为尺度
for iN=1:length(JN)
